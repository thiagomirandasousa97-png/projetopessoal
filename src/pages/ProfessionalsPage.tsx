...
export default function ProfessionalsPage() {
  ...
  const load = async () => {
    try {
      ...
      for (const apt of appointments ?? []) {
        ...
        statsMap[pid].totalRevenue += Number((svc as { price?: number } | null)?.price ?? 0);
        statsMap[pid].appointmentCount += 1;
      }
      setStats(statsMap);
    } catch (err) {
      console.error(err);
      toast.error("Nao foi possivel carregar profissionais.");
      setItems([]);
    }
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      console.error(error);
      toast.error("Nao foi possivel salvar o profissional.");
    }
  };
  ...
  return (
    <div>
      <PageHeader title="Profissionais" description="Cadastre e gerencie comissoes e especialidades.">
        <Dialog>
      ...
      {!isAdmin && (
        <p className="text-sm text-muted-foreground mb-4">
          Voce possui acesso somente leitura. Para cadastrar ou editar profissionais, solicite permissao de administrador.
        </p>
      )}
              ...
              <CardContent className="pt-5">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-primary mt-1">Comissao: {item.commissionPercent}%</p>
                    </div>
                  </div>
                ...
                {s && (
                  <div className="mt-3 pt-3 border-t border-border text-xs text-muted-foreground flex justify-between">
                    ...
                    {item.commissionPercent > 0 && <span>Comissao: {toCurrency(commission)}</span>}
                  </div>
                )}
              </CardContent>
            </Card>
